...
export async function searchCustomer(query: string, storeId?: string) {
  ...
  try {
    // 1. Search in new LoyaltyCustomer table
    const loyaltyCustomers = await prisma.loyaltyCustomer.findMany({
      where: {
        OR: [
    ...
    if (loyaltyCustomers.length > 0) {
      return loyaltyCustomers.map(c => ({
        id: c.id,
        firstName: c.nom || 'Client',
        lastName: 'Fidélité',
        phone: c.phone,
        loyalty: {
          points: c.points
    ...
    // 2. Fallback to search in old Customer table
    ...
    return oldCustomers.map(c => ({
      ...
      lastName: c.lastName,
      phone: c.phone,
      loyalty: c.loyalty ? { points: c.loyalty.points } : null
    }))
  } catch {
    return []
  }
